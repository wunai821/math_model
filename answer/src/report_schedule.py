...
- `shapes` 从附件读取全部C类计划，先确认新增装备确实使用统一的频宽、时长、间隔和次数。
- `occupied` 收集固定问题2方案占用的全部时频格；撤销计划不占资源。
- 两层候选循环完整枚举首次时间和频率起点，`isdisjoint` 删除与q2方案相撞的位置。
- `add_at_most_one` 保证同一时频格最多被一个新增候选占用，`maximize(sum(variables))` 直接最大化新增数量。
- `q3/verify.py` 重新展开12次重复使用，独立检查窗口、参数和冲突。
...
- `candidates_for(p)` 为每台装备列出原样、频移、首次时间平移和撤销；只有C类再加入间隔变化，并过滤越界候选。
- `build_model(plans)` 用 `add_exactly_one` 保证每台装备恰选一种状态，用 `add_at_most_one` 保证同一时频格不被多个候选占用。
- `solve.py` 将撤销、调整数量和幅度成本放入七层目标，逐层求解并锁定前一层结果。
- `verify.py` 不复用模型内部的候选表，而是重新读取附件、展开全部重复时段并检查冲突。
- `reproduce.py` 用于从基准文件重新复测，确认结果不是缓存或手工表格造成的。
